...
const { newSequelizeSettingsRepository } = require('../repositories/settings');
...
function createRoute() {
  ...
  const dbUsername =
    process.env.DATABASE_USERNAME || dbConfig.username || 'admin';
  const dbPassword =
    process.env.DATABASE_PASSWORD || dbConfig.password || 'mysql';
  ...
  const settingsRepository = newSequelizeSettingsRepository(sequelize);

  const studentService = newStudentService(
    studentRepository,
    majorRepository,
    settingsRepository
  );
  const teacherService = newTeacherService(teacherRepository);
  const reportService = newReportService(
    reportRepository,
  ...
  const classService = newClassService(
    classRepository,
    assignmentRepository,
  ...
  user_v1.get('/', userController.get.bind(userController));
  // user_v1.put('/', authenticate, userController.update.bind(userController));
  user_v1.put('/', userController.update.bind(userController));
  ...
  class_v1.post(
    '/:id/assignment',
    classController.addAssignment.bind(classController)
  );
  class_v1.put(
    '/assignment/:id/verify',
    classController.verifyAssignment.bind(classController)
  );
  class_v1.post(
    '/:id/meeting',
    classController.addMeeting.bind(classController)
  );
  class_v1.post(
    '/:id/evaluate',
    authenticate,
    verifyRoles(ROLES.TEACHER),
    classController.evaluate.bind(classController)
  );
  ...
  assignment_v1.put(
    ...
    authenticate,
    verifyRoles(ROLES.TEACHER),
    assignmentController.evaluate.bind(assignmentController)
  );
